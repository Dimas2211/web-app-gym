...
## Platform — FASE VI-E3: FE01 + CCFE03 runtime creation pipeline (cerrado)

Migra creación (`PENDING_GENERATION`), generación de JSON (`GENERATED`) y validación de schema MH (`SCHEMA_VALIDATED`) de **FE 01 y CCFE 03 emitidos desde Sale** para que un RUNTIME_CLIENT haga todo el pipeline en su propia DB física, sin tocar Prisma global. NO firma, NO transmite, NO MH, NO MariaDB — igual que las fases previas de este bloque. Ver `docs/modules/platform-phase-6e3-fe-ccfe-runtime-creation.md` para el detalle completo.

- **Servicios migrados a `db` explícito** (default `= prisma`, comportamiento preservado para callers no migrados): `createPendingDteForSale` (`dte-outgoing.service.ts`, usa `db.$transaction` — Sale/DteOutgoingDocument/DteIssuerConfig/DteCorrelative todo dentro de la misma transacción runtime), `generateFeJsonForDte`, `generateCcfeJsonForDte`, `validateDteJsonSchema`, y el resolver territorial compartido `validateDteAddressCodes`/`resolveDteMunicipality` (`dte-territory.resolver.ts`) — Municipality es RUNTIME_REFERENCE (mismo patrón que UnitOfMeasure en VI-D9: códigos oficiales estables, pero cada runtime DB tiene su propia copia seedeada; el lookup debe ir contra la DB efectiva, no la global).
- **`createPendingDteForPurchase` (FSE 14) deliberadamente NO tocado** — sigue en Prisma global, mismo archivo, fuera de alcance de VI-E3.
- **6 entry points migrados a `requireOperationalContext`/`getDteApiContext`** (module `fiscal.dte`, `write: true`): `create-pending-dte-for-sale.action.ts`, `create-pending-dte-simple.action.ts` (sus lookups de Sale/DteIssuerConfig antes usaban Prisma global directo — ahora usan `context.client`), `generate-fe-json-for-sale.action.ts`, `generate-ccfe-json-for-sale.action.ts`, `validate-dte-json-schema.action.ts`, y `POST /api/dte/outgoing/pending` (mismo patrón que `issuer-config/route.ts` de VI-E2B).
- Support Session bloqueada estructuralmente en los 6 (write:true → READ_ONLY antes de tocar DB). Sale ownership sigue fail-closed: todo lookup de Sale filtra por `tenant_id`+`location_id` efectivos, nunca por lo enviado en el body.
- Reglas fiscales, cálculo de IVA, mapeo FE/CCFE y protección de duplicados/correlativos: sin cambios de comportamiento — solo threading de `db`.
- Sin cambios de schema/migraciones. 35 tests nuevos (295 en el módulo DTE, 702 en el repo). `tsc`/`lint`/`build` verdes.
- **Fuera de alcance, explícitamente pendiente para VI-E4**: FEX 11, FSE 14 (purchase), NC 05 — sus builders/actions siguen en Prisma global sin cambios. Firma y transmisión (VI-E5) tampoco se tocaron.

## Platform — FASE VI-E4A: FSE14 + FEX11 runtime creation pipelines (cerrado)

Migra creación (`PENDING_GENERATION`), generación de JSON (`GENERATED`) y validación de schema MH (`SCHEMA_VALIDATED`) de **FSE 14 (emitido desde Purchase) y FEX 11 (emitido desde Sale de exportación)** para que un RUNTIME_CLIENT haga todo el pipeline en su propia DB física, mismo criterio que VI-E3 para FE01/CCFE03. NO firma, NO transmite, NO MH, NO MariaDB, NO toca NC05 (diferido a VI-E4B). Ver `docs/modules/platform-phase-6e4a-fse-fex-runtime-creation.md` para el detalle completo.

- **FSE 14 — 5 archivos migrados a `db` explícito** (default `= prisma`, comportamiento preservado para callers no migrados): `createPendingDteForPurchase` (`dte-outgoing.service.ts`, ahora usa `db.$transaction` en vez del `prisma.$transaction` hardcodeado que VI-E3 dejó deliberadamente sin tocar), `generateFseJsonForPurchase` (`generate-fse-json.service.ts` — `buildFseJsonFromLoadedData` es función pura, sin acceso a Prisma, no requirió cambios), `generateAndPersistFseJsonForDte` (`generate-fse-json-pipeline.service.ts`), y las 2 Server Actions (`create-pending-dte-for-purchase.action.ts`, `generate-fse-json-for-purchase.action.ts`) migradas de `requireAdmin()` + resolución manual de `DteIssuerConfig`/`resolveCommercialEnforcementContext` a `requireOperationalContext(sessionUser, { module: "fiscal.dte", write: true })`, pasando `context.client` al servicio.
- **FEX 11 — 9 archivos migrados a `db` explícito**: `export-sale.service.ts` (`createForeignCustomer`, `configureUnitMhCode`, `loadActiveTestIssuerConfigOrError`, `createPendingExportDte` — usa `db.$transaction` en vez de `prisma.$transaction` —, `regenerateRejectedExportDte`, `createExportSale`; esta última ya pasaba `db` a `createSaleDraft`/`addSaleItemToDraft`/`confirmSale`, que aceptan el parámetro desde VI-D5, pero antes no lo hacía), `get-unit-mh-context.ts`, `search-export-products.ts`, `search-foreign-customers.ts` (las 3 queries de lookup del panel de exportación), `generate-fex-json.service.ts` (`generateFexJsonForSale`; `buildFexJsonFromLoadedData` es pura, sin cambios), `generate-fex-json-pipeline.service.ts` (`generateAndPersistFexJsonForDte`), y 3 Server Actions: `generate-fex-json-for-sale.action.ts` (de `requireAdmin()`+`getEffectiveLocationId()` a `requireOperationalContext`), `export-sale.actions.ts` (las 6 actions del formulario de venta de exportación, guard único `requireExportSession` migrado) y `export-sale-dte.actions.ts` (panel DTE del módulo — `loadExportDteState` ahora recibe `db`; las actions de firma/transmisión/entrega externa que delega —`signDteDocumentAction`/`transmitDteDocumentAction`/`deliverDteToExternalDbAction`— NO se tocaron, siguen fuera de alcance de esta fase).
- **`listDteCatalogItems`** (`list-dte-catalog-items.ts`, usado por `export-sale.service.ts` para CAT-027/028/029/031/015 y el catálogo de compatibilidad FEX v1) se clasificó `GLOBAL_REFERENCE` y NO se migró — es un catálogo de sistema sin `tenant_id` en el schema (`DteCatalogItem`), a diferencia de `UnitOfMeasure`/`Municipality` (`RUNTIME_REFERENCE`, con FK física local por runtime DB). Documentado explícitamente, no es un gap.
- **8 entry points migrados a `requireOperationalContext`** (module `fiscal.dte` para las 2 actions de FSE14 y la de FEX11 JSON; module `commerce.sales` para `export-sale.actions.ts`/`export-sale-dte.actions.ts`, igual que su guard manual previo `resolveCommercialEnforcementContext(tenant_id, "commerce.sales")`). Support Session bloqueada estructuralmente en los 8 (`write: true` → READ_ONLY antes de tocar DB). Purchase/Sale ownership sigue fail-closed: todo lookup filtra por `tenant_id`+`location_id` efectivos, nunca por lo enviado en el body.
- Reglas fiscales, retención 1% IVA / renta de FSE14, cálculo FEX11 (0% exportación, seguro/flete, catálogo país FEX v1), y protección de duplicados/correlativos (`reserveDteControlNumber`, siempre con `tx` derivado del `db` efectivo): sin cambios de comportamiento — solo threading de `db`.
- Sin cambios de schema/migraciones. 16 tests nuevos (718 en el repo, antes 702). `tsc --noEmit`, `npm run lint` y `npm run build` verdes.
- **Fuera de alcance, explícitamente pendiente para VI-E4B**: NC 05 (Nota de Crédito) — depende del DTE original y requiere checks de ownership/estado referencial propios, no tocado. Firma y transmisión (VI-E5) tampoco se tocaron.

## Platform — FASE VI-E4B: NC05 runtime creation pipeline (cerrado)

Migra creación (`PENDING_GENERATION`), generación de JSON (`GENERATED`) y validación de schema MH (`SCHEMA_VALIDATED`) de **NC 05** (Nota de Crédito, emitida sobre un CCFE 03 ACCEPTED) para que un RUNTIME_CLIENT haga todo el pipeline en su propia DB física, mismo criterio que VI-E3/VI-E4A. NO firma, NO transmite, NO MH, NO MariaDB. Con esta fase, **NC05 es el último documento DTE productivo runtime-aware hasta SCHEMA_VALIDATED** — el bloque de creación (FE01/CCFE03/FSE14/FEX11/NC05) queda completo. Ver `docs/modules/platform-phase-6e4b-nc-runtime-creation.md` para el detalle completo.

- **2 servicios migrados a `db` explícito** (default `= prisma`, comportamiento preservado para callers no migrados): `createCreditNoteDteFromAcceptedCcfe` (`create-credit-note-dte.service.ts`, usa `db.$transaction` en vez del `prisma.$transaction` hardcodeado — lookup del CCFE original, bloqueo de duplicado F3-C24, issuer config, `reserveDteControlNumber` y create del NC + `DteDocumentRelation`, todo dentro de la misma transacción runtime), `generateNcJsonForDte` (`generate-nc-json.service.ts` — 4 reads + 1 update sobre `db`, y `validateDteAddressCodes` ahora recibe `db` explícito).
- **3 Server Actions migradas a `requireOperationalContext`** (module `fiscal.dte`, `write: true`): `create-credit-note-dte.action.ts`, `generate-nc-json.action.ts` (ninguna de las dos tenía guard de solo-lectura propio antes de esta fase — gap cerrado), y el orquestador `create-and-transmit-credit-note.action.ts` (reemplaza su blindaje manual `isRuntimeReadOnlyActive()` + `resolveCommercialEnforcementContext`). Los 3 primeros pasos del orquestador (crear NC, generar JSON, validar schema) reciben `context.client`; `signDteDocument`/`transmitDteDocument` (fuera de alcance, pendiente VI-E5) quedan intactos, solo reciben `tenantId`/`locationId`/`userId` ya resueltos por el contexto, sin `client` — siguen sobre Prisma global internamente, mismo comportamiento previo.
- Original DTE same-runtime invariant: el CCFE 03 que la NC modifica se busca dos veces (al crear la NC y al generar su JSON, vía `DteDocumentRelation`), ambas contra el mismo `db` runtime efectivo, siempre filtrado por `tenant_id`+`location_id` — un CCFE de otro tenant/runtime es indistinguible de "no existe" (fail closed).
- Reglas fiscales (NC05 crédito total V1, mapeo de `cuerpoDocumento`, cálculo de IVA/resumen), idempotencia (bloqueo de segunda NC activa excluyendo INVALIDATED/REJECTED) y protección de correlativos: sin cambios de comportamiento — solo threading de `db`.
- Sin cambios de schema/migraciones. 23 tests nuevos (741 en el repo, antes 718). `tsc --noEmit`, `npm run lint` y `npm run build` verdes.
- **Fuera de alcance, explícitamente pendiente para VI-E5**: firma (`sign-dte-document.*`) y transmisión (`transmit-dte-document.*`) de todos los documentos DTE (FE01/CCFE03/FSE14/FEX11/NC05) — siguen 100% sobre Prisma global.

## Platform — FASE VI-E5A: DTE signing runtime awareness (cerrado)

Migra la **firma** (`sign-dte-document.*`) de DTE — FE01, CCFE03, FSE14, FEX11, NC05 — para que un RUNTIME_CLIENT firme íntegramente contra su propia runtime DB: documento + `DteIssuerConfig` + `DteCredential` + `DteTransmissionLog` salen/entran de la MISMA base física, nunca solo del mismo `tenant_id`. Transmisión a MH (`transmit-dte-document.*`) sigue explícitamente **fuera de alcance** (VI-E5B) y sigue 100% sobre Prisma global. Ver `docs/modules/platform-phase-6e5a-dte-runtime-signing.md` para el detalle completo (matriz de entry points, decisión sobre el flujo combinado de NC, censo de Prisma global).

- **1 servicio migrado a `db` explícito** (default `= prisma`): `signDteDocument` (`sign-dte-document.service.ts`) — carga `DteOutgoingDocument` y (nuevo) `DteIssuerConfig` desde `db`, pasa `client: db` a `resolveDteSignerConfigForIssuer` (que ya aceptaba `client` desde VI-E4A/dte-credential.service.ts), y usa `db.$transaction` para persistir `SIGNED`/`retry_count`+`DteTransmissionLog` en ambos casos (éxito/fallo). Se agrega un check estructural nuevo: el `environment` de `DteIssuerConfig` debe coincidir con el del documento antes de resolver credenciales — bloquea cualquier mezcla TEST/PRODUCTION que antes solo dependía de la constraint única de la tabla.
- **2 Server Actions migradas a `requireOperationalContext`** (module `fiscal.dte`, `write: true`): `sign-dte-document.action.ts` (reemplaza `requireAdmin` + `getEffectiveLocationId` + `resolveCommercialEnforcementContext` manual) y `reopen-rejected-dte-for-resign.action.ts` (mismo reemplazo; el documento se reabre en `context.client`, la misma runtime DB donde después se firma). `reopen-signed-dte-for-resign.service.ts` también acepta `db` explícito por consistencia, pero **no tiene ninguna Server Action que lo invoque hoy** (dead code a nivel de entry point — se deja preparado, no se crea una action nueva fuera de alcance).
- **Orquestador NC05 combinado** (`create-and-transmit-credit-note.action.ts`, VI-E4B) — decisión Q, opción (A): `signDteDocument` ahora recibe `context.client` igual que los 3 pasos anteriores (crear/generar/validar), pero `transmitDteDocument` sigue intacto sobre Prisma global. Para que un RUNTIME_CLIENT nunca alcance esa transmisión global con datos fiscales propios, la action corta el flujo fail-closed inmediatamente después de firmar cuando `context.authScope === "RUNTIME_CLIENT"`: la NC queda `SIGNED` en su propia runtime DB y la action devuelve `stepFailed: "transmitir_deferred"`. PLATFORM_NATIVO conserva el flujo completo create+sign+transmit sin cambios.
- Support Session (`SUPPORT_RUNTIME`) sigue bloqueada por el mismo mecanismo ya certificado en VI-D2/E3/E4 (`requireOperationalContext({ write: true })` rechaza `READ_ONLY` antes de tocar el documento o el firmador). El runner F2-B2 (`support-dte-sign-runner.ts` / `signSupportDteAction`) es una feature de soporte **deliberadamente separada y ya blindada** (requiere `super_admin`, bloquea `PRODUCTION` explícitamente, exige confirmación textual + Safety Gate D0, solo LOCAL/TEST/SANDBOX) — no se tocó, no forma parte del pipeline RUNTIME_CLIENT certificado aquí.
- El adapter HTTP real (`dte-signer.adapter.ts`) y `transmit-dte-document.service.ts` **no se modificaron funcionalmente**. Ningún test de esta fase hace una llamada de red real al firmador ni a Hacienda — el adapter siempre se mockea.
- Sin cambios de schema/migraciones. 19 tests nuevos (760 en el repo, antes 741): `sign-dte-document.service.runtime-write.test.ts` (9, nuevo), `sign-dte-document.action.test.ts` (+5, reescrito para el nuevo patrón), `reopen-rejected-dte-for-resign.action.test.ts` (4, nuevo), `create-and-transmit-credit-note.action.test.ts` (+1, cobertura RUNTIME_CLIENT fail-closed). `tsc --noEmit`, `npm run lint` y `npm run build` verdes.
- **Fuera de alcance, explícitamente pendiente para VI-E5B**: transmisión a Hacienda (`transmit-dte-document.service.ts`, `MhDteTransmissionAdapter`, auth MH) de todos los documentos DTE — sigue 100% sobre Prisma global. `create-and-transmit-credit-note.action.ts` para RUNTIME_CLIENT queda deliberadamente detenido en `SIGNED` hasta que exista esa fase.

## Platform — FASE VI-E5B: DTE transmission runtime awareness (cerrado)

Migra la **transmisión a Hacienda** (`transmit-dte-document.*`) de DTE — FE01, CCFE03, FSE14, FEX11, NC05 — para que un RUNTIME_CLIENT transmita íntegramente contra su propia runtime DB: documento firmado + `DteIssuerConfig`/`DteCredential` (vía `MhAuthAdapter.credentialClient`) + `DteTransmissionLog` + ledger de metering (`DteFiscalMeteringReservation`) salen/entran de la MISMA base física. Capacidad comercial (`resolveCommercialEnforcementContext`) sigue resolviéndose siempre contra Control Plane — nunca runtime DB. Con esta fase, el ciclo de emisión completo (creación → firma → transmisión) queda runtime-aware para RUNTIME_CLIENT. Ver `docs/modules/platform-phase-6e5b-dte-runtime-transmission.md` para el detalle completo.

- **1 servicio migrado a `db` explícito** (default `= prisma`, mismo patrón que `sign-dte-document.service.ts` en VI-E5A): `transmitDteDocument` (`transmit-dte-document.service.ts`) — carga `DteOutgoingDocument` desde `db`, usa `db.$transaction` para las 3 transiciones finales (ACCEPTED/OBSERVED/REJECTED) y para la rama de error técnico/estado inesperado, pasa `runtimeDb: db` a `reserveDteFiscalCapacity` (antes hardcodeado a `prisma` global), y construye `MhDteTransmissionAdapter` con un `MhAuthAdapter({ credentialClient: db })` explícito — `resolveMhAuthCredentials` (ya runtime-aware desde IV-B.4) ahora sí recibe ese `db` en el flujo SEND real, cerrando el gap que dejaba credenciales MH resolviéndose siempre contra Prisma global sin importar el `db` del documento. El motor de metering (`dte-fiscal-metering.service.ts`) y el cache de tokens MH (keyed por `issuerConfigId:environment`, ya aislado desde F-DTE-ENV) no se modificaron — ya eran runtime-capable/runtime-safe.
- **1 Server Action migrada a `requireOperationalContext`** (module `fiscal.dte`, `write: true`): `transmit-dte-document.action.ts` (reemplaza `requireAdmin` + `getEffectiveLocationId` + `resolveCommercialEnforcementContext`/`assertOrganizationModule` manual, mismo reemplazo que `sign-dte-document.action.ts`). `transmitDteDocument` recibe `context.client` como segundo argumento.
- **Orquestador NC05 combinado** (`create-and-transmit-credit-note.action.ts`) — cierra la decisión Q dejada pendiente en VI-E5A: el flujo combinado ya NO se detiene después de firmar. `transmitDteDocument({...ctx}, context.client)` corre para RUNTIME_CLIENT igual que para PLATFORM_NATIVE, reutilizando el mismo service runtime-aware (sin duplicar implementación). El corte fail-closed `stepFailed: "transmitir_deferred"` se elimina.
- **Fuera de alcance, sin cambios** (AD): `assertDteContingencyTransmissionAllowed` sigue consultando `DteContingencyEventItem` sobre Prisma global — solo aplica a documentos `transmission_type_code="2"` (contingencia), que permanecen pendientes de migración en VI-E6 junto con reconciliación/invalidación/MariaDB. Documentos normales (`transmission_type_code="1"`, la inmensa mayoría) retornan `{ ok: true }` de ese guard sin tocar ninguna base de datos.
- Support Session (`SUPPORT_RUNTIME`) sigue bloqueada por el mismo mecanismo ya certificado en VI-D2/E3/E4/E5A (`requireOperationalContext({ write: true })` rechaza `READ_ONLY` antes de invocar `transmitDteDocument`, MH o el ledger de metering).
- Sin cambios de schema/migraciones. 9 tests nuevos (769 en el repo, antes 760): `transmit-dte-document.action.test.ts` (5, nuevo), `transmit-dte-document.service.runtime.test.ts` (4, nuevo, certifica routing de `db` vs Prisma global para documento/transacción/metering/credencial), `create-and-transmit-credit-note.action.test.ts` (reescrito, ya no certifica el corte fail-closed sino el flujo completo RUNTIME_CLIENT). `tsc --noEmit`, `npm run lint` y `npm run build` verdes.
- **Fuera de alcance, explícitamente pendiente para VI-E6**: reconciliación (`reconcile-dte-with-mh.action.ts`), invalidación, contingencia (guard de arriba) y delivery MariaDB — todos siguen sobre Prisma global/Control Plane sin cambios.

## Platform — FASE VI-E6A: DTE reconciliation + reopen/resign runtime awareness (cerrado, con gaps documentados)

Migra la **reconciliación con MH** (`reconcile-dte-with-mh.*`) para que un RUNTIME_CLIENT reconcilie íntegramente contra su propia runtime DB, y audita/certifica los flujos de reapertura para re-firma (`reopen-rejected-dte-for-resign.*`, `reopen-signed-dte-for-resign.service.ts`). Cierra además dos gaps de cobertura de tests heredados de VI-E5B (transmisión): aislamiento cross-tenant/location e integración de ambiente TEST/PRODUCTION. Contingencia (`assertDteContingencyTransmissionAllowed`), invalidación y delivery MariaDB quedan explícitamente FUERA de alcance (VI-E6B/C/E7).

- **`dte-reconciliation.service.ts` — ya era 100% runtime-aware** (diseño desde FASE IV-B.4, `runtimeDb: PrismaClient` requerido, sin `import { prisma }`). El único gap real estaba en la Server Action: **`reconcile-dte-with-mh.action.ts`** usaba el patrón pre-VI-E5A (`requireAdmin` + `getEffectiveLocationId` + `resolveCommercialEnforcementContext`/`isRuntimeReadOnlyActive` manual) y pasaba `runtimeDb: prisma` (el singleton global) en vez de una runtime DB por tenant. Migrada a `requireOperationalContext(sessionUser, { module: "fiscal.dte", write: true })`, igual que `reopen-rejected-dte-for-resign.action.ts` — ahora pasa `runtimeDb: context.client`. Test reescrito completo (15 casos, antes 13) certificando READ_ONLY (Support Session) y MODULE_DISABLED bloqueando ANTES de invocar el service, y que `runtimeDb` recibido es siempre `context.client`, nunca el prisma global.
- **`list-dte-query-history.ts`** (lectura de `DteTransmissionLog` filtrado por `operation_type="QUERY"` — no existe una tabla `DteQueryHistory` separada) — ya aceptaba `client` inyectable desde antes; se confirmó que su único caller real (`get-dte-outgoing-detail-by-id.ts` ← `GET /api/dte/outgoing/:id`) ya forwardea `ctx.client` (contexto runtime resuelto por `getDteApiContext`). Sin cambios necesarios.
- **`reopen-rejected-dte-for-resign.service.ts`** — ya era runtime-capable desde VI-E5A (`db: PrismaClient = prisma` inyectable, transacción única, preconditions REJECTED + sin `reception_stamp` + `codigoMsg` 802 verificado contra el log SEND real). Sin cambios de código; se le agregó su **primer test dedicado** (`reopen-rejected-dte-for-resign.service.test.ts`, 8 casos nuevos — antes solo se certificaba indirectamente vía mock en el test de la action): flujo normal, las 4 preconditions fail-closed, y aislamiento cross-tenant/cross-location.
- **`reopen-signed-dte-for-resign.service.ts` — confirmado código muerto a nivel de entry point** (`REOPEN_SIGNED_ENTRYPOINT = NONE`): grep exhaustivo confirma cero callers en todo el repositorio, sin Server Action ni ruta que lo invoque. El código en sí ya es runtime-capable (mismo patrón `db: PrismaClient = prisma`) desde que se escribió. Por instrucción explícita de esta fase, **no se inventó una Server Action/UI nueva** — se deja documentado tal cual, listo para cuando exista el caso de uso real (certificado de firmador reemplazado).
- **Metering durante reconciliación/reopen — comportamiento verificado, sin cambios de código**: `finalizePendingReservationByDocument` (único camino PENDING→CONSUMED específico de reconciliación, exclusivo de `dte-reconciliation.service.ts`) ya funcionaba correctamente. Los flujos de reopen (rejected y signed) **no tocan la tabla de metering en absoluto** — es correcto por diseño: un REJECTED ya tuvo su reserva liberada (RELEASED) en el momento del rechazo original (dentro de `transmit-dte-document.service.ts`); reabrir para re-firma no debe re-reservar, esa re-reserva ocurre naturalmente la próxima vez que `transmitDteDocument` llame `reserveDteFiscalCapacity`, que ya maneja la transición RELEASED→PENDING como "reapertura" (rama existente desde antes de esta fase). No se agregó ninguna lógica nueva de metering en los services de reopen — hubiera sido una regresión conceptual.
- **Contingency guard — confirmado, sin tocar (deuda ya documentada)**: `assertDteContingencyTransmissionAllowed` sigue leyendo `DteContingencyEventItem`/`DteContingencyEvent` desde el Prisma global sin ningún parámetro `db`/`runtimeDb` inyectable — su caller (`transmit-dte-document.service.ts`, ya migrado desde VI-E5B) no le pasa `db` porque el guard no lo acepta. Pendiente explícitamente para VI-E6C, tal como VI-E5B ya lo había dejado registrado.
- **2 gaps de cobertura de VI-E5B cerrados**:
  1. Aislamiento cross-tenant/cross-location de transmisión — nuevo bloque de tests en `transmit-dte-document.service.runtime.test.ts` (+2) que simula el scoping real de `findFirst({ where: { id, tenant_id, location_id } })` y certifica `MhDteTransmissionAdapter`/`MhAuthAdapter`/`reserveDteFiscalCapacity`/`$transaction` con 0 llamadas cuando el tenant/location del caller no coincide con el dueño real del documento.
  2. Integración de ambiente TEST/PRODUCTION — nuevo archivo `dte-transmission.environment-integration.test.ts` (3 tests) que usa las clases REALES `MhAuthAdapter` + `MhDteTransmissionAdapter` (nunca mockeadas como constructor, solo el boundary de red vía `fetch` y el boundary de credenciales vía `resolveMhAuthCredentials`) y certifica el chain completo: `environment` del documento → URL de auth MH → URL de recepción MH → código de ambiente en el payload (`00`=TEST, `01`=PRODUCTION) → propagación del mismo `credentialClient` a `resolveMhAuthCredentials` (same-runtime credential). Antes solo existía el test unitario aislado de `resolveDteMhUrls` (`dte-mh.config.test.ts`).
  3. Aislamiento cross-tenant/cross-location de reconciliación — 2 tests nuevos en `dte-reconciliation.service.test.ts` (findFirst ya scoped por diseño desde IV-B, ahora certificado explícitamente) + 2 tests de integración de ambiente TEST/PRODUCTION en el mismo archivo (URL de consulta MH seleccionada correctamente según `dteDoc.environment`).
- Support Session (`SUPPORT_RUNTIME`) sigue bloqueada por el mismo mecanismo certificado desde VI-D2 (`requireOperationalContext({ write: true })` rechaza `READ_ONLY` antes de invocar `reconcileDteWithMh`, MH o el ledger de metering) — sin código nuevo, la migración de la action lo hereda automáticamente.
- Sin cambios de schema/migraciones. 20 tests nuevos (789 en el repo, antes 769): `reconcile-dte-with-mh.action.test.ts` (reescrito, 15 casos), `dte-reconciliation.service.test.ts` (+4: 2 cross-tenant/location, 2 integración de ambiente), `transmit-dte-document.service.runtime.test.ts` (+2 cross-tenant/location), `dte-transmission.environment-integration.test.ts` (3, nuevo), `reopen-rejected-dte-for-resign.service.test.ts` (8, nuevo). `tsc --noEmit` limpio (solo la deuda preexistente ya documentada en `generate-nc-json.service.runtime-write.test.ts`, sin relación con esta fase). `npm run lint` sin errores nuevos. `npm run build` PASS.
- **Fuera de alcance, explícitamente pendiente**: invalidación (VI-E6B), contingencia productiva — el guard sigue sobre Prisma global (VI-E6C), delivery MariaDB runtime-aware (VI-E7). `DTE_TRANSMISSION_RUNTIME_READY` permanece `PARTIAL` hasta VI-E6C.

## Platform — FASE VI-E6B: DTE invalidation runtime awareness (cerrado, con deuda documentada)

Migra el flujo completo de **invalidación DTE** (create → sign → transmit) para que un RUNTIME_CLIENT invalide íntegramente contra su propia runtime DB, mismo patrón certificado en VI-E3/E4/E5/E6A. Contingencia, MariaDB general y el orquestador `create-and-transmit-credit-note.action.ts` NO se tocaron.

- **3 servicios migrados a `db: PrismaClient = prisma` explícito**: `createInvalidationEvent` (`create-invalidation-event.service.ts`), `signInvalidationEvent` (`sign-invalidation-event.service.ts`), `transmitInvalidationEvent` (`transmit-invalidation-event.service.ts`). Los tres leen/escriben `DteOutgoingDocument`, `DteIssuerConfig`, `DteInvalidationEvent` y `DteTransmissionLog` SIEMPRE contra el mismo `db` recibido — nunca el Prisma global cuando hay uno explícito.
- **Consistencia de ambiente documento↔emisor agregada** en `createInvalidationEvent` y `signInvalidationEvent` (no existía antes) — mismo criterio estructural agregado en VI-E5A a `sign-dte-document.service.ts`: rechaza mezclar TEST/PRODUCTION entre `DteOutgoingDocument.environment` y `DteIssuerConfig.environment` antes de construir el JSON o llamar al firmador.
- **`signInvalidationEvent` — cambio de mecanismo de credenciales (spec ítem J)**: antes resolvía el firmador EXCLUSIVAMENTE desde `DTE_SIGNER_NIT`/`DTE_SIGNER_PASSWORD` (env global, nunca por emisor). Ahora reusa `resolveDteSignerConfigForIssuer` (el mismo mecanismo certificado en VI-E5A para `sign-dte-document.service.ts`), pasando `client: db` — si existe una `DteCredential` activa para el emisor/ambiente, se usa esa; si no, cae al mismo fallback global de siempre. Nunca cruza ambientes ni bases físicas.
- **`transmitInvalidationEvent`** — construye `MhInvalidationTransmissionAdapter(new MhAuthAdapter({ credentialClient: db }))`, mismo patrón que `transmit-dte-document.service.ts` (VI-E5B): la credencial MH (si existe una por emisor/ambiente) se resuelve de la MISMA runtime DB que el documento/evento, nunca del Control Plane.
- **4 Server Actions migradas a `requireOperationalContext`**: `create-invalidation-event.action.ts`, `sign-invalidation-event.action.ts`, `transmit-invalidation-event.action.ts` y el orquestador productivo **`create-sign-transmit-invalidation.action.ts`** (reemplaza su guard manual `isRuntimeReadOnlyActive()` + `resolveCommercialEnforcementContext` por el helper único — mismo comportamiento, una sola fuente de verdad). Los tres pasos del orquestador reciben SIEMPRE el mismo `context.client`.
- **`deliver-invalidation-to-external-db.service.ts`** — se le agregó `client?: PrismaClient` opcional (mismo patrón preexistente en `deliver-dte-to-external-db.service.ts`), haciendo el servicio capaz de LEER desde runtime DB. **El entry point productivo (`deliver-invalidation-to-external-db.action.ts`) NO se tocó** — sigue sobre `requireAdmin` + Prisma global, deliberadamente, porque VI-E7 es la fase reservada para el cierre general de MariaDB/entrega externa. `DTE_MARIADB_DELIVERY_RUNTIME_READY` permanece `PARTIAL`.
- **Metering — confirmado sin cambios de código**: ninguno de los 3 servicios de invalidación importa ni llama `dte-fiscal-metering.service.ts`. Un DTE ya ACCEPTED que se invalida sigue consumiendo su cupo `fiscal.dte.monthly_issued` — no se libera. Comportamiento preexistente, ahora con regresión test explícita.
- **Deuda documentada, NO corregida en esta fase (spec ítem L)**: `transmitInvalidationEvent` marca el evento `SENT`/DTE `INVALIDATION_PENDING` en una transacción ANTES de llamar a MH (igual que `transmit-dte-document.service.ts` desde antes de VI-E5B). Un crash entre ese commit y la persistencia del resultado de MH deja el evento en `SENT` y el DTE en `INVALIDATION_PENDING` sin resolución automática — a diferencia de la transmisión normal, `dte-reconciliation.service.ts` (VI-E6A) **NO tiene ninguna rama que lea `INVALIDATION_PENDING`**, así que hoy no existe un camino de recuperación automatizado para este estado específico (`INVALIDATION_PENDING_CRASH_RECOVERY_SAFE = NO`). Recuperación esperada hoy: reintento manual de `transmitInvalidationEvent` sobre el mismo evento (SIGNED sigue siendo el estado real tras revertir), o extender `dte-reconciliation.service.ts` en una fase futura para cubrir invalidación — no se hizo aquí por ser un rediseño fuera del alcance de VI-E6B.
- Sin cambios de schema/migraciones. 37 tests nuevos (826 en el repo, antes 789): `create-invalidation-event.service.runtime-write.test.ts` (8, nuevo), `sign-invalidation-event.service.runtime-write.test.ts` (8, nuevo), `transmit-invalidation-event.service.runtime-write.test.ts` (12, nuevo, incluye aislamiento cross-tenant/location), `create-sign-transmit-invalidation.action.test.ts` (9, nuevo). `tsc --noEmit` limpio salvo la deuda preexistente ya documentada en `generate-nc-json.service.runtime-write.test.ts`. `npm run lint` sin errores nuevos. `npm run build` PASS.
- **Fuera de alcance, explícitamente pendiente**: contingencia productiva (VI-E6C), delivery MariaDB runtime-aware end-to-end (VI-E7).

## Platform — FASE VI-E6C: DTE contingency runtime awareness (cerrado, con deuda documentada)

Migra el flujo completo de **contingencia DTE** (create event → build/persist JSON → sign → transmit) y el guard `assertDteContingencyTransmissionAllowed` (que gatea `transmitDteDocument` para `transmission_type_code="2"`) para que un RUNTIME_CLIENT opere íntegramente contra su propia runtime DB, mismo patrón certificado en VI-E5A/E5B/E6A/E6B. Arquitectura Dedicated DB sin cambios — no se introdujo Shared/Hybrid.

- **5 servicios migrados a `db: PrismaClient = prisma` explícito**: `createContingencyEvent` (`create-contingency-event.service.ts`), `buildAndPersistContingencyEventJson` (`persist-contingency-event-json.service.ts` — el builder puro `build-contingency-event-json.service.ts` no toca DB y no requiere `db`), `signContingencyEvent` (`sign-contingency-event.service.ts`), `transmitContingencyEvent` (`transmit-contingency-event.service.ts`) y **`assertDteContingencyTransmissionAllowed`** (`assert-dte-contingency-transmission-allowed.service.ts`) — este último es el gap crítico que dejaban documentado VI-E5B y VI-E6A: el guard leía `DteContingencyEventItem`/`DteContingencyEvent` desde el Prisma global sin ningún `db` inyectable, aunque su caller (`transmitDteDocument`) ya era runtime-aware desde VI-E5B.
- **`transmit-dte-document.service.ts`** — el call site del guard (paso 3b) ahora pasa `db` explícito: `assertDteContingencyTransmissionAllowed({...}, db)`. Cierra el cross-DB gap: antes, un RUNTIME_CLIENT transmitiendo un DTE `transmission_type_code="2"` ejecutaba TODO el resto del flujo contra su runtime DB pero la validación de contingencia seguía consultando el Prisma global (Control Plane / PLATFORM_NATIVE por defecto).
- **`signContingencyEvent` — cambio de mecanismo de credenciales (mismo criterio que VI-E6B ítem J)**: antes resolvía el firmador EXCLUSIVAMENTE desde `DTE_SIGNER_NIT`/`DTE_SIGNER_PASSWORD` (env global). Ahora reusa `resolveDteSignerConfigForIssuer` (mecanismo certificado en VI-E5A/VI-E6B), pasando `client: db` — si existe una `DteCredential` activa para el emisor/ambiente del documento asociado, se usa esa; si no, cae al mismo fallback global de siempre. Nunca cruza ambientes ni bases físicas.
- **`assertDteContingencyTransmissionAllowed` — enforcement de ownership movido al `where` del query** (antes: `findMany` amplio por `dte_document_id` + filtro `tenant_id`/`location_id`/`status`/`contingency_type_code` en memoria vía `.some()`). Ahora el `where` filtra `contingency_event: { tenant_id, location_id, status: "ACCEPTED", contingency_type_code }` directamente en la consulta — el enforcement vive en la query misma, no en JS después de traer filas potencialmente ajenas. Sin cambio de regla de negocio, solo dónde se aplica.
- **`CONTINGENCY_PRODUCT_ENTRYPOINT = NONE`** — auditoría confirmó que no existe ninguna Server Action ni ruta/página productiva que invoque `createContingencyEvent`/`buildAndPersistContingencyEventJson`/`signContingencyEvent`/`transmitContingencyEvent`; los únicos callers son los scripts dev (`src/modules/commerce/dte/dev/verify-contingency-*.ts`). Por instrucción explícita de esta fase, **no se inventó una Server Action/UI nueva** — los 5 servicios quedan certificados como runtime-capable, listos para cuando exista un orquestador productivo real (análogo a `create-sign-transmit-invalidation.action.ts`).
- **`DteOutgoingStatus.CONTINGENCY_PENDING` confirmado código muerto/no usado** en el flujo real — existe solo en enum/tipos/UI (labels de filtro), pero ningún servicio lo escribe nunca. El estado real de un DTE contingente sigue gobernado por `transmit-dte-document.service.ts` (SIGNED→ACCEPTED/REJECTED, igual que un DTE normal); lo que cambia es que `assertDteContingencyTransmissionAllowed` bloquea la transmisión si no hay un `DteContingencyEvent` ACCEPTED que lo cubra.
- **Metering — confirmado sin cambios de código**: ninguno de los 5 servicios de contingencia importa `dte-fiscal-metering.service.ts`. El consumo de `fiscal.dte.monthly_issued` sigue ocurriendo únicamente dentro de `transmitDteDocument` (idéntico para `transmission_type_code="1"` y `"2"` — la transmisión del Evento de Contingencia en sí, vía `/fesv/contingencia`, es un canal MH separado que no mueve el ledger de metering).
- **Deuda documentada, NO corregida en esta fase (mismo patrón que VI-E6B ítem L)**: `transmitContingencyEvent` marca `DteContingencyEvent.status = "SENT"` en una transacción ANTES de llamar a MH. Un crash entre ese commit y la persistencia del resultado de MH deja el evento en `SENT` sin resolución automática — `dte-reconciliation.service.ts` (VI-E6A) **NO tiene ninguna rama que lea `DteContingencyEvent`**, así que no existe un camino de recuperación automatizado (`CONTINGENCY_CRASH_RECOVERY_SAFE = NO`). A diferencia de invalidación, este `SENT` no deja ningún campo del `DteOutgoingDocument` asociado en un estado intermedio — el evento de contingencia es una entidad separada, así que el DTE en sí permanece en su estado normal (SIGNED, pendiente de transmisión real) durante el crash window. Recuperación esperada hoy: reintento manual de `transmitContingencyEvent` sobre el mismo evento tras revertir manualmente a `SIGNED` en DB si quedó huérfano en `SENT`.
- Sin cambios de schema/migraciones. 31 tests nuevos (857 en el repo, antes 826): `create-contingency-event.service.runtime-write.test.ts` (4, nuevo), `persist-contingency-event-json.service.runtime-write.test.ts` (5, nuevo), `sign-contingency-event.service.runtime-write.test.ts` (6, nuevo), `transmit-contingency-event.service.runtime-write.test.ts` (6, nuevo), `assert-dte-contingency-transmission-allowed.service.runtime-write.test.ts` (9, nuevo, incluye aislamiento cross-tenant/location/tipo/ambiente), `transmit-dte-document.service.runtime.test.ts` (+1: certifica que el guard recibe `db`). `tsc --noEmit` limpio salvo la deuda preexistente ya documentada en `generate-nc-json.service.runtime-write.test.ts`. `npm run lint` sin errores nuevos. `npm run build` PASS.
