...
- Reconciliación (`reconcile-dte-with-mh.action.ts`), invalidación, contingencia y delivery MariaDB runtime-aware (VI-E6) — únicos pasos del ciclo DTE que siguen 100% sobre Prisma global tras VI-E5B. Hasta entonces, solo `DELIVER_EXTERNAL` está allowlisted como excepción de escritura runtime; esos flujos solo corren vía runner de soporte (F2-B2, LOCAL/TEST/SANDBOX) o PLATFORM_NATIVE, no desde UI operativa RUNTIME_CLIENT.
