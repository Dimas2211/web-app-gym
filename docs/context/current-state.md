...
- Transmisión a Hacienda runtime-aware (VI-E5B, `transmit-dte-document.service.ts`) — único paso del ciclo DTE que sigue 100% sobre Prisma global para RUNTIME_CLIENT tras VI-E5A. Hasta entonces, solo `DELIVER_EXTERNAL` está allowlisted como excepción de escritura runtime; el resto del ciclo post-firma solo corre vía runner de soporte (F2-B2, LOCAL/TEST/SANDBOX) o PLATFORM_NATIVE, no desde UI operativa RUNTIME_CLIENT.
