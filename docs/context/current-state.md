...
- Contingencia productiva (VI-E6C — el guard `assertDteContingencyTransmissionAllowed` sigue sobre Prisma global) y delivery MariaDB runtime-aware end-to-end (VI-E7) — únicos pasos del ciclo DTE que siguen 100% sobre Prisma global/Control Plane tras VI-E6B (creación/firma/transmisión, reconciliación, reopen/resign e invalidación ya cerrados). Hasta entonces, solo `DELIVER_EXTERNAL` está allowlisted como excepción de escritura runtime; esos flujos solo corren vía runner de soporte (F2-B2, LOCAL/TEST/SANDBOX) o PLATFORM_NATIVE, no desde UI operativa RUNTIME_CLIENT.
