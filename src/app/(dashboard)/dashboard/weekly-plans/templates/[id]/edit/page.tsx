...
export default async function EditWeeklyPlanTemplatePage({ params }: Props) {
  ...
  // cliente" redirige ANTES de cargar el registro/selectores. La query de
  // sucursales usa context.client — antes usaba el prisma singleton directo (BUG).
  ...
  try {
    ...
    const [template, sports, goals, branches] = await Promise.all([
      getWeeklyPlanTemplateById(id, effectiveUser, context.client),
