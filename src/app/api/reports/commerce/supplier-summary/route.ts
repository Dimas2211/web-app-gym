...
// ─────────────────────────────────────────────────────────────────
// GET /api/reports/commerce/supplier-summary
//
// Resumen por proveedor: compras agrupadas.
...
import { getSupplierSummaryReport } from "@/modules/commerce/reports/queries/get-supplier-summary-report";
...
export async function GET(req: NextRequest) {
  ...
  // Dato expuesto = compras agrupadas por proveedor (commerce.purchases).
  ...
  const reportCtx = await resolveReportApiContext(user.tenant_id, "commerce.purchases", user);
  ...
  try {
    ...
    const rows = await getSupplierSummaryReport(
      {
    ...
    console.error("[supplier-summary]", err);
