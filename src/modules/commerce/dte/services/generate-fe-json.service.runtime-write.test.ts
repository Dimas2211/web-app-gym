// ─────────────────────────────────────────────────────────────────
// commerce/dte — generate-fe-json.service.runtime-write.test.ts
//
// FASE VI-E3 — generateFeJsonForDte acepta un `db` explícito y usa esa
// misma DB para TODOS los reads (DteOutgoingDocument, Sale,
...
vi.mock("@/lib/db/prisma", () => ({
  prisma: new Proxy(
    ...
    {
      get() {
        throw new Error("RUNTIME_UNSAFE: generateFeJsonForDte tocó el Prisma global.");
      },
    },
  ),
}));
...
import { generateFeJsonForDte } from "./generate-fe-json.service";
...
describe("generateFeJsonForDte — FASE VI-E3 (runtime db injection)", () => {
  it("usa exclusivamente context.client (db) para DteOutgoingDocument/Sale/DteIssuerConfig", async () => {
    const { db } = buildFakeRuntimeDb();

    const result = await generateFeJsonForDte("dte-1", "tenant-1", "loc-1", "u1", db as never);
  ...
  it("propaga el mismo db al resolver territorial (validateDteAddressCodes)", async () => {
    ...
    await generateFeJsonForDte("dte-1", "tenant-1", "loc-1", "u1", db as never);
  ...
  it("sin db explícito -> usa Prisma global por defecto (comportamiento preservado)", async () => {
    await expect(generateFeJsonForDte("dte-1", "tenant-1", "loc-1", "u1")).rejects.toThrow("RUNTIME_UNSAFE");
  });
});
