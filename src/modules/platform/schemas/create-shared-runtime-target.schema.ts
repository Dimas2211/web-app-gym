// ─────────────────────────────────────────────────────────────────
// platform — create-shared-runtime-target.schema.ts
//
// Validación Zod para crear un PlatformSharedRuntimeTarget. El
// password es obligatorio en creación y se cifra en la action — nunca
// se persiste en plano ni se devuelve al cliente.
// ─────────────────────────────────────────────────────────────────

import { z } from "zod";

const DATABASE_PROFILE_ENVIRONMENTS = ["LOCAL", "SANDBOX", "TEST", "STAGING", "PRODUCTION"] as const;
const DATABASE_PROVIDERS = ["POSTGRESQL", "SUPABASE", "NEON", "RENDER", "LOCAL_POSTGRES", "OTHER"] as const;
const DATABASE_SSL_MODES = ["DISABLE", "PREFER", "REQUIRE"] as const;

export const createSharedRuntimeTargetSchema = z.object({
  label: z
    .string({ required_error: "El nombre del Shared Runtime es requerido." })
  ...
  provider: z.enum(DATABASE_PROVIDERS).default("POSTGRESQL"),
  ...
  ssl_mode: z.enum(DATABASE_SSL_MODES).default("PREFER"),
});

export type CreateSharedRuntimeTargetInput = z.infer<typeof createSharedRuntimeTargetSchema>;
