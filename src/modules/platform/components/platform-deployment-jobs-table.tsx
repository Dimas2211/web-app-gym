...
// ─────────────────────────────────────────────────────────────────
// platform — platform-deployment-jobs-table.tsx
//
// Tabla de Deployment Jobs con filtros básicos.
// ─────────────────────────────────────────────────────────────────
...
export function PlatformDeploymentJobsTable({ jobs }: Props) {
  if (jobs.length === 0) {
    return (
      <p className="text-sm text-zinc-400 italic py-6 text-center">
        Sin deployment jobs registrados.
      </p>
    );
  }
        ...
        <tbody>
          {jobs.map((job) => (
            <tr key={job.id} className="border-b border-zinc-100 hover:bg-zinc-50 transition-colors">
              ...
              <td className="py-2.5">
                <Link
                  href={`/dashboard/platform/deployments/${job.id}`}
