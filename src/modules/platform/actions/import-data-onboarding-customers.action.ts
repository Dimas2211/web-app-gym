...
//
// E1C-C: Server action para importar clientes desde un archivo
// Excel hacia la base de datos cliente seleccionada (PlatformDatabaseProfile).
//
// Flujo:
//  1. requireSuperAdmin() obligatorio.
//  2. Validar parámetros: datasetKey === "customers", policy === "CREATE_ONLY".
...
//  7. Re-ejecutar análisis DB-aware server-side (nit/dui/nombre como llave).
...
//  - Confirmación textual obligatoria en EXECUTE: "IMPORT CUSTOMERS".
...
export async function importDataOnboardingCustomersAction(
  ...
  try {
    ...
    if (datasetKey !== "customers") {
      return {
        success: false,
        error: `E1C-C solo permite importar dataset 'customers' desde esta action. Recibido: '${datasetKey}'.`,
      };
    }
    ...
    if (profile.environment === "PRODUCTION") {
      return {
        ...
        error: "Importación de clientes bloqueada en PRODUCTION.",
      };
    }
    ...
    const safetyInput: DatabaseExecutionSafetyInput = {
      ...
      expectedConfirmationText:          mode === "EXECUTE"
                                           ? IMPORT_CUSTOMERS_CONFIRMATION_TEXT
                                           : undefined,
      ...
      try {
        await prisma.platformDeploymentLog.create({
          data: {
            ...
            action:          "RUN_IMPORT",
            status:          "SUCCESS",
            notes:           `Import clientes E1C-C — perfil: ${profile.label} — created: ${imp.created}`,
            metadata: {
    ...
    if (mode === "DRY_RUN") {
      const dry = runnerResult as import("../types/platform.types").CustomersImportDryRunResult;
      return {
        success:        true,
        mode:           "DRY_RUN",
        profileLabel:   profile.label,
    ...
    const imp = runnerResult as import("../types/platform.types").CustomersImportResult;
    return {
      success:        true,
      mode:           "EXECUTE",
      profileLabel:   profile.label,
    ...
    const raw  = err instanceof Error ? err.message : "Error inesperado al importar clientes.";
