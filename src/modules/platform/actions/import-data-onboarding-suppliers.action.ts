...
// ─────────────────────────────────────────────────────────────────
// platform — import-data-onboarding-suppliers.action.ts
//
// E1C-C: Server action para importar proveedores desde un archivo
// Excel hacia la base de datos cliente seleccionada (PlatformDatabaseProfile).
//
// Flujo:
//  1. requireSuperAdmin() obligatorio.
//  2. Validar parámetros: datasetKey === "suppliers", policy === "CREATE_ONLY".
//  3. Hard block: PRODUCTION bloqueado siempre en E1C-C.
...
//  7. Re-ejecutar análisis DB-aware server-side (nit/nombre como llave).
...
//  - Confirmación textual obligatoria en EXECUTE: "IMPORT SUPPLIERS".
...
import { runSuppliersImport }
  from "../lib/data-onboarding/import-runners/suppliers-import-runner";
...
export async function importDataOnboardingSuppliersAction(
  formData: FormData,
): Promise<SuppliersImportActionState> {
  try {
    // ── 1. Autenticación ──────────────────────────────────────────
    ...
    if (datasetKey !== "suppliers") {
      return {
        success: false,
        error: `E1C-C solo permite importar dataset 'suppliers' desde esta action. Recibido: '${datasetKey}'.`,
      };
    }
    if (importPolicy !== "CREATE_ONLY") {
      return {
        success: false,
        error: `E1C-C solo permite política CREATE_ONLY. Recibida: '${importPolicy}'.`,
      };
    }
    ...
    const safetyInput: DatabaseExecutionSafetyInput = {
      ...
      expectedConfirmationText:          mode === "EXECUTE"
                                           ? IMPORT_SUPPLIERS_CONFIRMATION_TEXT
                                           : undefined,
    ...
    if (errorRows.length > 0) {
      return {
        ...
        error:
          ...
          "(duplicado por NIT/nombre u otros datos inválidos). " +
          "Corrija el archivo y vuelva a intentar.",
      };
    }

    // Solo filas CREATE son aceptables en E1C-C CREATE_ONLY
    ...
    if (nonCreateRows.length > 0) {
      return {
        ...
        error:
          ...
          "El archivo contiene proveedores que ya existen en la base destino.",
      };
    }
      ...
      try {
        await prisma.platformDeploymentLog.create({
          data: {
            ...
            action:          "RUN_IMPORT",
            status:          "SUCCESS",
            notes:           `Import proveedores E1C-C — perfil: ${profile.label} — created: ${imp.created}`,
            metadata: {
    ...
    if (mode === "DRY_RUN") {
      const dry = runnerResult as import("../types/platform.types").SuppliersImportDryRunResult;
      return {
        success:        true,
        mode:           "DRY_RUN",
        profileLabel:   profile.label,
    ...
    const imp = runnerResult as import("../types/platform.types").SuppliersImportResult;
    return {
      success:        true,
      mode:           "EXECUTE",
      profileLabel:   profile.label,
    ...
    const raw  = err instanceof Error ? err.message : "Error inesperado al importar proveedores.";
