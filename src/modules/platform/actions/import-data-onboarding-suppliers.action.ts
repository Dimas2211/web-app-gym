"use server";

// ─────────────────────────────────────────────────────────────────
// platform — import-data-onboarding-suppliers.action.ts
//
// E1C-C: importa proveedores desde Excel hacia la base runtime de UNA
// organización (Shared o Dedicated). Solo escribe en suppliers.
//
// SHARED-OPS-PARITY-1: el flujo completo (super_admin, resolución
// organizationId → tenant_id → runtime server-side, Commercial
// Enforcement, re-parseo, análisis DB-aware tenant-scoped, política de
// PRODUCTION, runner transaccional, log) vive en runDataOnboardingImport.
// Esta action solo aporta la configuración del dataset.
//
// Reglas: CREATE_ONLY (no actualiza, no elimina, no upsert), sin import
// parcial, confirmación textual obligatoria en EXECUTE ("IMPORT SUPPLIERS"
// y, en PRODUCTION, + organization.code), sin exponer credenciales.
// ─────────────────────────────────────────────────────────────────

import { runDataOnboardingImport }
  from "../lib/data-onboarding/run-data-onboarding-import";
import { runSuppliersImport }
  from "../lib/data-onboarding/import-runners/suppliers-import-runner";
import { IMPORT_SUPPLIERS_CONFIRMATION_TEXT }
  from "../lib/data-onboarding/import-runners/suppliers-import.constants";
import type {
  SuppliersImportActionState,
  SuppliersImportDryRunResult,
  SuppliersImportResult,
} from "../types/platform.types";

export async function importDataOnboardingSuppliersAction(
  formData: FormData,
): Promise<SuppliersImportActionState> {
  return runDataOnboardingImport<SuppliersImportDryRunResult, SuppliersImportResult>(formData, {
    datasetKey:           "suppliers",
    phase:                "E1C-C",
    datasetLabel:         "proveedores",
    baseConfirmationText: IMPORT_SUPPLIERS_CONFIRMATION_TEXT,
    analysisErrorMessage: (n) =>
      `El análisis server-side detectó ${n} fila(s) con error ` +
      "(duplicado por NIT/nombre u otros datos inválidos). " +
      "Corrija el archivo y vuelva a intentar.",
    nonCreateMessage: (n) =>
      `${n} fila(s) no pueden crearse ` +
      "(política CREATE_ONLY: solo se permiten registros nuevos). " +
      "El archivo contiene proveedores que ya existen en la base destino.",
    unexpectedErrorMessage: "Error inesperado al importar proveedores.",
    run: ({ client, parsedPreview, dbAwareResult, tenantId, isDryRun }) =>
      runSuppliersImport({
        parsedPreview,
        dbAwareResult,
        prismaClient: client,
        tenantId,
        isDryRun,
      }),
    created: (imp) => imp.created,
    buildLogMetadata: (imp) => ({
      created:   imp.created,
      skipped:   imp.skipped,
      errors:    imp.errors,
      totalRows: imp.totalRows,
    }),
  });
}
