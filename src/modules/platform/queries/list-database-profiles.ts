...
export async function listDatabaseProfiles(
  ...
  const rows = await prisma.platformDatabaseProfile.findMany({
    ...
    select: {
      ...
      direct_db_name:    true,
      direct_db_user:    true,
      // direct_encrypted_password: omitido intencionalmente
  ...
  return rows.map((r) => ({
    ...
    direct_db_host:   r.direct_db_host,
    direct_db_port:   r.direct_db_port,
    direct_db_name:   r.direct_db_name,
    direct_db_user:   r.direct_db_user,
    direct_ssl_mode:  r.direct_ssl_mode as PlatformDatabaseProfileItem["direct_ssl_mode"],
