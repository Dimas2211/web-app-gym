// ─────────────────────────────────────────────────────────────────
// platform — list-deployment-jobs.ts
//
// Lista filtrada de PlatformDeploymentJob.
// Solo super_admin (validación en la page/action que llama).
// ─────────────────────────────────────────────────────────────────

import { prisma } from "@/lib/db/prisma";
import type {
  PlatformDeploymentJobFilters,
...
function buildWhere(filters: PlatformDeploymentJobFilters) {
  return {
    ...(filters.organization_id && { organization_id: filters.organization_id }),
    ...(filters.job_status       && { job_status:       filters.job_status }),
    ...(filters.environment      && { target_environment: filters.environment }),
  };
}

export async function listDeploymentJobsQuery(
  filters: PlatformDeploymentJobFilters = {},
): Promise<PlatformDeploymentJobItem[]> {
